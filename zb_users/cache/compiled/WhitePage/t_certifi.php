...
<body>
    ...
    <div class="layui-row layui-col-space15">
      <div class="layui-col-md12">
        <div class="layui-card">
          <div class="layui-card-header">实名验证</div>
          <div class="layui-card-body">

        <?php if ($isidcard == 1) { ?>
            <blockquote class="layui-elem-quote">待审核</blockquote>
        <?php }elseif($isidcard == 2) {  ?>
            <blockquote class="layui-elem-quote">已验证</blockquote>
            
        <?php }else{  ?>
            <?php if ($isidcard == 3) { ?>
            <blockquote class="layui-elem-quote">未通过</blockquote>
            <?php } ?>
            <form id="signup-form" class="layui-form" action="" lay-filter="component-form-element">
              <div class="layui-row layui-col-space10 layui-form-item">
                ...
                <div class="layui-col-lg12">
                  ...
                  <div class="layui-input-block">
                    <input type="text" name="idcard" lay-verify="required" placeholder="" autocomplete="off" class="layui-input">
                  </div>
                </div>
              </div>
  ...
  layui.config({
    ...
    form.on('submit(component-form-element)', function(data){
        admin.req({
        url: zbloghost + 'common/certifi.php'
        ,type:'post'
        ...
        ,done: function(res){
          layer.msg('修改成功', {
            ...
            location.href = '<?php  echo $host;  ?><?php  echo $zbp->Config('YtUser')->YtUser_Certifi;  ?>';
          });
        },
        error:function(res){
            layer.msg($(res.responseText).find('member value string').text());
            $("#reg_verfiycode").attr("src",basehost+"zb_system/script/c_validcode.php?id=Certifi&amp;tm="+Math.random());
        }
      });
      return false;
    });
