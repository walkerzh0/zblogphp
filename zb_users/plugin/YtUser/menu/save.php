...
if(GetVars('type','GET') == 'contribute' ){
    $zbp->Config('YtUser')->contribute_on = GetVars('contribute_on','POST');
    $zbp->Config('YtUser')->contribute_jf = GetVars('contribute_jf','POST');
    $zbp->SaveConfig('YtUser');
	$zbp->SetHint('good','修改成功');
	Redirect('./contribute.php');
}

if(GetVars('type','GET') == 'invite' ){    	 			 		    			   		
    $zbp->Config('YtUser')->inv_on = GetVars('inv_on','POST');      		            			 
    $zbp->Config('YtUser')->inv_oneon = GetVars('inv_oneon','POST');     	 			 	    	 						    	   		
    $zbp->Config('YtUser')->inv_jifen = (int)GetVars('inv_jifen','POST');
    $zbp->SaveConfig('YtUser');
	$zbp->SetHint('good','修改成功');
	Redirect('./invite.php');
}

if(GetVars('type','GET') == 'testingdel' ){
	global $zbp;    	   	 	     		 			       		 	 		
	$sql = $zbp->db->sql->Delete($GLOBALS['tysuer_Table'],array(array('=','tc_uid',0)));     	 				      	  				     	  			 
	$zbp->db->Delete($sql);    								       			 	    	  				 
	Redirect('./testing.php');      	 	  	    					 	     		 		  	
}

if(GetVars('type','GET') == 'sms' ){    	 			 		    			   		    				 		 
    $zbp->Config('YtUser')->sms_on = GetVars('sms_on','POST');      		            			      		 				
    $zbp->Config('YtUser')->sms_limit = GetVars('sms_limit','POST');     	 			 	    	 						      	  	 	
    $zbp->Config('YtUser')->sms_count = GetVars('sms_count','POST');      	   		    		 	 	  
    $zbp->Config('YtUser')->sms_sign = GetVars('sms_sign','POST');                 	   			
    $zbp->Config('YtUser')->sms_code = GetVars('sms_code','POST');     			 	 	     			  	 
    $zbp->Config('YtUser')->sms_verifycode = GetVars('sms_verifycode','POST');      	  	      	  		  	
    $zbp->SaveConfig('YtUser');    		    		    	  	 		     	  	  	 
	$zbp->SetHint('good','修改成功');     	 		 	     	   		 	     	   		 
	Redirect('./sms.php');      	  	      		 		 		    				 			
}    		 	 	 	    		 		       			 	  	
     	    	     			    	      		 		 
if(GetVars('type','GET') == 'payment' ){    	 				       	   	             	
    $zbp->Config('YtUser')->payment = GetVars('payment','POST');       				     	  			 	     	 	  	 
    $zbp->Config('YtUser')->brand = GetVars('brand','POST');       	  	     		  				    		 	  	 
    $zbp->Config('YtUser')->pricetemp1 = GetVars('pricetemp1','POST');    	  	 			     						     		 	 	  
    $zbp->Config('YtUser')->pricetemp2 = GetVars('pricetemp2','POST');    			 			     	  		  	    	 	   	 
    $zbp->Config('YtUser')->pricetemp3 = GetVars('pricetemp3','POST');          		    	  		  	    		  				
    $zbp->Config('YtUser')->pricetemp5 = GetVars('pricetemp5','POST');    	   				     						     	   		 	
    $zbp->Config('YtUser')->pricetemp6 = GetVars('pricetemp6','POST');     	  				    			 	 	     		  	 		
    $zbp->Config('YtUser')->pricetemp7 = GetVars('pricetemp7','POST');     	 		 	     				 			    	 					 
    $zbp->SaveConfig('YtUser');    	 	   	     		 		        	   	 	
	$zbp->SetHint('good','修改成功');    		    		    	 	    	    	  	 			
	Redirect('./payment.php');    	 		  		    	    		      		  		 
}      		 			    	   	       						  
    		     	     				 	      		 		  
if(GetVars('type','GET') == 'download' ){    			 	 		      		  	       	 		 	
    $zbp->Config('YtUser')->isdownload = GetVars('isdownload','POST');       		         		 		        			 	
    $zbp->Config('YtUser')->download_notice = GetVars('download_notice','POST');    	 		  		       	 	 	    	 	  		 
    $zbp->SaveConfig('YtUser');      	  	 	    		 	  	     		  	  	
	$zbp->SetHint('good','修改成功');    	  		 	        		  	    				    
...
if(GetVars('type','GET') == 'base' ){
	$zbp->Config('YtUser')->user_ytecn = GetVars('user_ytecn','POST');
    ...
    $zbp->Config('YtUser')->sign = GetVars('sign','POST');           	    		   	          			 
...
if(GetVars('type','GET') == 'setidvip' ){     	 		  	     		 				    	 	 			 
	...
	if (isset($zbp->members[$uid])){     		  	       		   	      				 		
		...
		if(GetVars('reseta','POST')=='getvip'){     	     	      	  	      		 					
			...
			if($Vipendtime<time()){$Vipendtime=time();}    		  		 	      		   	      	 		 	
			$add = $Vipendtime + 86400*(int)$add;     		 	        				  	    		 	  	 
...
if(GetVars('type','GET') == 'testing' ){    	 	 	 	     		 	        		 		 	 
	global $zbp;
	YtUser_DB_ADD(YtUser_ReplacePre($tysuer_Table),"tc_rootid","int(11) NOT NULL DEFAULT 0",true); 
	YtUser_DB_ADD(YtUser_ReplacePre($tyactivate_Table),"reg_uptime","int(11) NOT NULL DEFAULT 0",true);
	YtUser_DB_ADD(YtUser_ReplacePre($typrepaid_Table),"tc_uptime","int(11) NOT NULL DEFAULT 0",true);
	YtUser_DB_ADD(YtUser_ReplacePre($YtConsume_Table),"cs_class","int(2) NOT NULL DEFAULT 0",true);
	YtUser_DB_ADD(YtUser_ReplacePre($YtVerification_Table),"vf_ip","varchar(15)",true);
    YtUser_DB_MOD(YtUser_ReplacePre($YtVerification_Table),"vf_ip","varchar(15) NOT NULL",true);
	YtUser_DB_ADD(YtUser_ReplacePre($YtVerification_Table),"vf_code","varchar(15)",true);
	YtUser_DB_ADD(YtUser_ReplacePre($YtVerification_Table),"vf_account","varchar(100)",true);
...
if(GetVars('type','GET') == 'rewrite' ){    		  			       	  			    		  	  	
	...
	$zbp->Config('YtUser')->YtUser_Register_u = GetVars('YtUser_RWURL','POST').'/Register/u/';
	...
	$zbp->Config('YtUser')->YtUser_Index = GetVars('YtUser_RWURL','POST').'/index.html';  
	}else{           	    	  	   	        	 	 
	...
	$zbp->Config('YtUser')->YtUser_Index = '?userindex';
	}    			 	       	  	           			  
    $zbp->SaveConfig('YtUser');    	 		  	      	 	  		    	  			  
	$zbp->SetHint('good','修改成功');     	 	 	 	     	   	 	     		  			
