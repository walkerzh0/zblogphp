...
if($zbp->Config('YtUser')->sms_on==1){      	   	       		 		     		 	 		 
    $mobile=trim(GetVars('mobile','POST'));      			  	    	     		           	
    $code=trim(GetVars('mobilecode','POST'));    		     	    			    	     							
    ...
    if($array){    		  	 	     					 	     		    	 
        ...
        if($code != $ver->Code){
            $zbp->ShowError('短信验证码不正确!!!');die();
        }else{
            if ($ver->Send < $enddayStartTime) {
                $zbp->ShowError('短信验证码已过期!!!');die();
            }
        }	      	        	       			 		 	
    }else{
        $zbp->ShowError('请输入短信验证码!!!');die();			 		
    }    		   			
    $member->Name=$mobile;      	  			    							     		 		 	 
}else{    						          	 		      	  		 
...
if ($zbp->Config('YtUser')->inv_on == 1){
    $rootid=(int)GetVars('rootid','POST');
    $memberrid= new Member();
    $memberrid->LoadInfoByID($rootid);
    $member->roodid=$memberrid->ID;
}else{
    $member->roodid=0;
}
